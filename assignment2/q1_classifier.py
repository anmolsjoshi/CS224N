...
class SoftmaxModel(Model):
    ...
    def add_prediction_op(self):
        ...
        #self.W = tf.Variable(tf.random_normal(shape=(self.config.n_features, self.config.n_classes), dtype=tf.float32))
        #self.b = tf.Variable(tf.zeros(shape=(self.config.n_classes, 1), dtype=tf.float32))
        #pred = softmax(tf.matmul(self.input_placeholder, self.W) + self.b)
