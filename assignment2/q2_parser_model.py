...
class ParserModel(Model):
    ...
    def create_feed_dict(self, inputs_batch, labels_batch=None, dropout=0.0):
    ...
    def add_embedding(self):
        ...
        """
        ### YOUR CODE HERE
        pretrained_embeddings = tf.nn.embedding_lookup(tf.constant(self.pretrained_embeddings), self.input_placeholder)
        embeddings = tf.reshape(pretrained_embeddings, [-1, self.config.n_features * self.config.embed_size])
        ### END YOUR CODE
        return embeddings
    ...
    def add_loss_op(self, pred):
        ...
        """
        ### YOUR CODE HERE
        loss = tf.reduce_mean(tf.nn.softmax_cross_entropy_with_logits(labels=self.labels_placeholder, logits=pred), axis=0)
        ### END YOUR CODE
        return loss
