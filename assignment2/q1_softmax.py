...
def softmax(x):
    ...
    #numer = tf.exp(tf.subtract(x, tf.transpose(tf.expand_dims(tf.reduce_max(x, axis=1), 0))))
    #denom = tf.reduce_sum(numer, axis=1)
    #out = numer/denom
...
def cross_entropy_loss(y, yhat):
    ...
    out = tf.reduce_sum(-tf.log(tf.reduce_sum(tf.multiply(tf.to_float(y), yhat), axis=1)))
    #y = tf.to_float(y)
